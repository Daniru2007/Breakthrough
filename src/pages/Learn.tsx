...
const units = [
  {
    ...
    lessons: [
      ...
      { type: 'speaking', completed: true, link: "./speaking" }, //questions
      { type: 'game', completed: true, link:"./game" }, //game
    ],
  },
  {
    id: 2,
                ...
                <div key={unit.id} className="unit-section">
                  ...
                  <div className="lesson-path">
                    {unit.lessons.map((lesson, index) => (
                        <Link to={lesson.link} key={index} className="lesson-item">
                          <div
                              className={`lesson-node ${
                                  lesson.completed ? 'completed' : ''
                              } ${unit.isLocked ? 'locked' : ''}`}
                          >
                            {lesson.type === 'game' && <Gamepad2 size={24} />}
                            {lesson.type === 'start' && <CirclePlay size={24} />}
                            {lesson.type === 'quiz' && <NotebookPen size={24} />}
                            {lesson.type === 'speaking' && <AudioLines size={24} />}
